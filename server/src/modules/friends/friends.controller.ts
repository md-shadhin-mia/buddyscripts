...
export async function getFriends(req: Request, res: Response) {
  ...
  const result = await friendsService.getFriends(req.user!.id, { cursor, take: take ? Number(take) : undefined })
  res.json({ status: "success", data: result })
}

export async function unfriend(req: Request<{ id: string }>, res: Response) {
  await friendsService.unfriend(req.user!.id, req.params.id)
  res.json({ status: "success", message: "Unfriended" })
}

export async function getSuggestions(req: Request, res: Response) {
  const suggestions = await friendsService.getSuggestions(req.user!.id)
